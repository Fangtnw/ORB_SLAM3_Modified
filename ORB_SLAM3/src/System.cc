...
void System::SavePointCloud(const string &filename){
    ...
    if(vMaps.empty()) {
        cout << endl << "No maps available!" << endl;
        return;
    }
    ...
    f << "map_id, pos_x, pos_y, pos_z" << endl;
    f << fixed;
    ...
    for (Map* pMap : vMaps) {
        ...
        if(vpMPs.empty()){
            cout << endl << "Map ID " << pMap->GetId() << " has no points!" << endl;
            continue;
        }
        ...
        for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
            ...
            f << pMap->GetId() << ", " << pos(0) << ", " << pos(1) << ", " << pos(2) << endl;
        }
    }

    f.close();

    // Optionally save reference points to a separate file
    f.open(("ref_" + filename).c_str());
    if (!f.is_open()) {
        cout << "Failed to open file: " << "ref_" + filename << endl;
        return;
    }
    f << "map_id, pos_x, pos_y, pos_z" << endl;
    f << fixed;
    ...
    for (Map* pMap : vMaps) {
        set<MapPoint*> spRefMPs(pMap->GetReferenceMapPoints().begin(), pMap->GetReferenceMapPoints().end());
